...
class GViTEncoder(nn.Module):
    def __init__(self,infeats,hidfeats,outfeats,heads=8,dropout=0.1,activation=nn.LeakyReLU()):
        ...
        """
        super().__init__()
        self.norm1 = nn.LayerNorm(infeats)
        self.norm2 = nn.LayerNorm(hidfeats * heads + infeats)
...
if __name__ == "__main__":
    ...
    encoder = GViTEncoder(6,16,16)
    H = embedding(X)
    assert H.shape == (10,16,6)
    Y = gal(H)
    assert Y.shape == (10,16,128)
    Y = encoder(H)
    assert Y.shape == (10,16,6+16*8+16)
