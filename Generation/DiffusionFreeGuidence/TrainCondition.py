...
def eval(modelConfig: Dict):
    ...
    with torch.no_grad():
        ...
        sampledImg, sampledImg_list= sampler(noisyImage, labels)
        sampledImg = sampledImg * 0.5 + 0.5  # [0 ~ 1]   
        save_image(sampledImg, os.path.join(
            modelConfig["sampled_dir"],  modelConfig["sampledImgName"]), nrow=modelConfig["nrow"])
        if modelConfig["gif_generation"]==True:
            for i in range(len(sampledImg_list)):
                sampledImg_list[i] = sampledImg_list[i]*0.5+0.5
                img_name = str(i)+'.png'
                save_image(sampledImg_list[i], os.path.join(
                    modelConfig["gif_dir"], img_name), nrow=modelConfig["nrow"])
